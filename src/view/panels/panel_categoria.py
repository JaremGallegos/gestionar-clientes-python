...
class PanelCategoriaLaboralDesign:
    def __init__(self, panel_principal):
        self._barra_superior = ctk.CTkFrame(panel_principal)
        self._barra_superior.pack(side = ctk.TOP, fill = 'both', expand = False)
        self._barra_inferior = ctk.CTkFrame(panel_principal)
        self._barra_inferior.pack(side = ctk.BOTTOM, fill = 'both', expand = True)
        self._label_titulo = ctk.CTkLabel(self._barra_superior, text = "Página Categoría Laboral", height = 20)
